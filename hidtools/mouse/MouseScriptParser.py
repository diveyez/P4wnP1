...
#		print "line: " + line
		...
		if first_word.upper() not in self.valid_commands:
			return None
		# remove linebreaks
		line = line.replace("\r\n","").replace("\n","")
		# remove comments
...
#			print "Parts {0}".format(parts)
	...
	def DO_BUTTONS(self, argv):
		...
		self.mouse.button1 = argv[0] != "0"
		self.mouse.button2 = argv[1] != "0"
		self.mouse.button3 = argv[2] != "0"
	...
	def executeScript(self, script):
		if len(script) > 0:
			commands = []
			commands.extend(
			    extracted for l in script if (extracted := self.extract_command(l)))
